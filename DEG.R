...
files <- list.files()
dbf.files <- files[grep(".csv", files, fixed=T)]
...
for (i in dbf.files)
{
  tryCatch(               
    ...
    expr = {
      ...
      counts_data <- read.csv(paste(id,'.csv', sep = ''), header = TRUE, row.names = 1)
      counts_data <- read.csv("counts_all_LNC.csv", header = TRUE, row.names = 1)
      
      
      
      # read in sample info
      condition <- c( "C", "C", "C","P","P","P","R","R","R","G","G","G")
      ...
      write.csv(data,paste(id, "_Up_down_SIG_genes.csv"),row.names = T)
